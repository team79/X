...
if ImageInit:
    ...
    for i in range(DataBlockNum):
        dirname = os.path.join(FLAGS.buckets, "Mk1501Image" + str(i+1) + ".txt")
    ...
    for i in range(DataBlockNum):
        dirname = os.path.join(FLAGS.buckets, "Mk1501Label" + str(i+1) + ".txt")
        x = file_io.read_file_to_string(dirname)
        x = np.fromstring(x)
...
for times in range(1):
    ...
    for i in range(2000):
        ...
        _ = sess.run([ train_op ], feed_dict={ image_holder : tmpimg, label_holder : tmplabel })
        if i % 100 == 0:
            t2 = sess.run(loss, feed_dict={ image_holder : tmpimg, label_holder : tmplabel })
