...
def time_tensorflow_run(session, target, info_string):
    ...
    for i in range(num_batches + num_steps_burn_in):
        ...
        inputs = np.zeros([BatchSize, ImageHeight, ImageWidth, 3])
        _ = session.run(target, feed_dict={image_holder : inputs})
...
if ImageInit:
    ...
    img_probe = np.zeros([ProbeLen,ImageHeight, ImageWidth,3])
    label_probe = np.zeros([ProbeLen])
    DataBlockNum = 8
    ...
    DataBlockNum = 4
    for i in range(DataBlockNum):
        dirname = os.path.join(FLAGS.buckets, "Mk1501TestGalleryImage" + str(i+1) + ".txt")
        x = file_io.read_file_to_string(dirname)
        x = np.fromstring(x)
...
print(time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()) + ":  " + "probe label size : ") 
print(label_probe.shape)
print(time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()) + ":  " + "gallery image size : ") 
print(img_gallery.shape)
print(time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()) + ":  " + "gallery label size : ") 
print(label_gallery.shape)
...
for times in range(1):
    print("**************************************************************\n" * 2)
    print(time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()) + ":  " + " the " + str(times + 1) + "th mk1501 test begin :")
    # if TrainInit == True:
    #     print(time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()) + ":  " + "init model variable")
    #     tf.global_variables_initializer().run()
    #     print(time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()) + ":  " + "init model variable done!")
    # else:
    print(time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()) + ":  " + "load model variable")
    ...
    for i in range(10):    
        ...
        tmpimg = img_gallery[(GalleryLen-BatchSize):(GalleryLen)]
        t1 = sess.run([FinalLocal], feed_dict = { image_holder : tmpimg } )
        feature_gallery[(GalleryLen-BatchSize):(GalleryLen)] = t1[0]
